...
A partir da técnica de brainstorm para compreender melhor as funcionalidades presentes na solução de software, foram elicitados os seguintes requisitos:
...
| RF-ING-03 | O sistema deve permitir a submissão combinada de texto e imagem na mesma consulta                                                          | Média      |
| RF-ING-04 | O sistema deve validar o formato e o tamanho máximo dos arquivos enviados (limite de 10 MB por imagem)                                     | Baixa      |
| RF-ING-05 | O sistema deve apresentar ao usuário, após o veredito, um questionário de no máximo 6 perguntas sobre a percepção de veracidade da notícia | Média      |
| RF-ING-06 | O sistema deve permitir que o usuário pule o questionário sem prejuízo do uso da aplicação                                                 | Baixa      |
...
| RF-MCP-02 | O sistema deve disponibilizar a tool `fact_check_claim` que recebe texto e retorna claims correspondentes, publisher, rating e URL da fonte           | Média      |
| RF-MCP-03 | O sistema deve disponibilizar a tool `check_image_origin` que recebe uma imagem e retorna URLs de imagens similares e contexto de origem              | Média      |
...
| RF-MCP-05 | O sistema deve disponibilizar a tool `analyze_text_image_consistency` que retorna descrição do conteúdo visual e inconsistências entre texto e imagem | Baixa      |
| RF-MCP-06 | O sistema deve disponibilizar a tool `get_user_profile` que retorna a persona/cluster atual do usuário                                                | Média      |
| RF-MCP-07 | O sistema deve disponibilizar a tool `record_feedback` que persiste as respostas do questionário                                                      | Baixa      |
| RF-MCP-08 | O sistema deve executar as tools de verificação (RF-MCP-02 a 05) em paralelo sempre que possível                                                      | Baixa      |
| RF-MCP-09 | O sistema deve tratar falhas em tools individuais sem derrubar o fluxo, retornando evidência "indisponível" para a ferramenta afetada                 | Baixa      |
...
| RF-VER-02 | O sistema deve registrar como "inconclusivo" quando a Fact Check API não retornar claims correspondentes                                                                      | Baixa      |
| RF-VER-03 | O sistema deve realizar busca reversa de imagem para detectar reutilização de imagem em contextos diferentes                                                                  | Média      |
| RF-VER-04 | O sistema deve analisar imagens submetidas por meio de LLM multimodal (vision), retornando descrição do conteúdo, plausibilidade de manipulação e inconsistências com o texto | Média      |
...
| RF-VER-06 | O sistema deve registrar todas as evidências brutas retornadas pelas ferramentas, com timestamp, para auditoria                                                               | Baixa      |

### Módulo de Perfilamento de Usuário

| ID        | Requisito                                                                                                                                                                                                            | Prioridade |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| RF-PER-01 | O sistema deve armazenar as respostas do questionário vinculadas ao usuário e à notícia avaliada                                                                                                                     | Baixa       |
...
| RF-PER-04 | O sistema deve executar clustering (K-means ou Gaussian Mixture) sobre os vetores de usuário, com k entre 3 e 4                                                                                                      | Média       |
| RF-PER-05 | O sistema deve rotular cada cluster como uma persona (ex.: "Crédulo emocional", "Cético verificador", "Passivo-scroll", "Confiante em autoridade")                                                                   | Baixa     |
| RF-PER-06 | O sistema deve validar a qualidade do clustering por silhouette score e registrar o resultado                                                                                                                        | Baixa      |
| RF-PER-07 | O sistema deve reexecutar o clustering periodicamente (job agendado) à medida que novas respostas chegam                                                                                                             | Baixa      |
| RF-PER-08 | O sistema deve reordenar a apresentação da justificativa do veredito conforme a persona do usuário (adaptação da explicação)                                                                                         | Baixa       |
